...
public class Validacion {
    ...
    public int validarClave (String clave) {
       int valorBarra = 0;
        if(clave.length()>=8){
            valorBarra=100; // se puede usar un contains para determinar que el nombre de usuario no este en la contraseña
        } else {
            if(clave.length()< 8){
            valorBarra = 30;
            }
 ...
 public String buscarUsuario(String usuario,String clave, boolean tipo){ // es mi nombre y apellido
     String nombreyApellido = "";
     List<Object> listaUsuario = (List<Object>) leeryEscribirarchivos.leerArchivo("listaUsuarios");
     for (Object listaUsuario1 : listaUsuario) {
         Usuario u = (Usuario) listaUsuario1;
         
            if (u.getUsuario().equals(usuario) && u.getContraseña().equals(clave)&& u.isTipo() == tipo){
