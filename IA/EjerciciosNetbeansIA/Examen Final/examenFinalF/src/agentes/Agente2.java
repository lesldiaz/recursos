    ...
    class Comportamiento extends Behaviour {
        ...
        public void action() {
            ...
            if(!v.isVisible()){
                ...
                ArrayList<double[]>lista=v.getLista();
                //System.out.println(lista.size());
                if (lista.size()!=0){
                    v.dispose();
                    String rs = recorreLista(lista);
                    enviarMensaje(rs, "Agente3", "convA2A3");
                    bandera=true;
                    doDelete();
                }
        ...
        public String recorreLista(ArrayList<double[]> lista){
           String result ="";
            for (int i = 0; i < lista.size(); i++) {
                double [] res = lista.get(i);
                    if (i==2){
                   result = result +res[0]+","+res[1]+","+res[2]; 
                   }else{
                    result = result+res[0]+","+res[1]+","+res[2]+",";
                   } 
